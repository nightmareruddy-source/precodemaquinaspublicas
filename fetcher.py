...
def ingest_keyword(keyword: str) -> int:
    ...
    with Session(engine) as session:
        for item in iter_results(payload):
            ...
            item_name = as_text(item.get("objetoCompra")) or as_text(item.get("objeto")) or category
            organ_name = extract_organ_name(item)
            municipality = extract_municipality(item)
            link_processo = as_text(item.get("linkProcesso"))
            record = MachineRecord(
                source="pncp",
                source_url=str(source_url)[:500],
                source_document_url=link_processo[:500] if link_processo else None,
                item_category=category,
                item_name=item_name[:255],
                organ_name=organ_name,
                municipality=municipality,
                supplier_name=as_text(item.get("nomeRazaoSocialFornecedor")) or as_text(item.get("fornecedor")),
                contract_type=extract_contract_type(joined, item),
                process_number=as_text(item.get("sequencialCompra")) or as_text(item.get("numeroControlePNCP")),
            ...
            if upsert_record(session, record):
                inserted += 1
        session.commit()
    return inserted
